""" Aerosol spec utilities, copied from pyoptimind. """
...
def lognorm_x_pdf(
    ...
    """Return x^ * n(x), where n is the normalized lognormal PDF."""
    return np.asarray(x) * lognorm_pdf(x, r_med, shape, mode_factors)
...
def lognorm_x_cdf(
    ...
    """Integral of x * n(x) from 0 to x, where n(x) is the normalized lognormal PDF."""

    def _integral(xsup: float) -> float:
        from scipy.integrate import quad
        return quad(lognorm_x_pdf, 0.0, xsup, args=(r_med, shape, mode_factors))[0]
...
def get_mean_diam_from_spec(spec):
    numer = lognorm_x_cdf(spec.rmax, spec.median, spec.shape) -\
            lognorm_x_cdf(spec.rmin, spec.median, spec.shape)
    denom = lognorm_cdf(spec.rmax, spec.median, spec.shape) -\
            lognorm_cdf(spec.rmin, spec.median, spec.shape)
    return numer/denom
